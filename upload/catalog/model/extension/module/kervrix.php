...
class ModelExtensionModuleKervrix extends Model
{
    ...
    {
        $this->db->query("INSERT IGNORE INTO " . DB_PREFIX . "kervrix_crm (order_id) VALUES ('" . (int)$order_id . "');");
    ...
    {
        $query = $this->db->query("SELECT order_id FROM " . DB_PREFIX . "kervrix_crm WHERE order_id='" . (int)$order_id . "' LIMIT 1");
    ...
    public function getOrder($order_id)
    {
        ...
        if (is_array($order_data)) {
            ...
            $order_data['products'] = $this->model_extension_module_kervrix->getOrderProducts($order_id);
            $order_data['totals'] = $this->model_extension_module_kervrix->getOrderTotals($order_id);
        }
    ...
    public function getOrders($data = array())
    {
        ...
        $allowed_orderby = array('o.order_id', 'o.date_added', 'o.order_status_id', 'o.total');
        $orderby = isset($data['orderby']) && in_array($data['orderby'], $allowed_orderby) ? $data['orderby'] : 'o.order_id';
        $sort = isset($data['sort']) && strtoupper($data['sort']) === 'ASC' ? 'ASC' : 'DESC';
